 ...
 *
 */
public class IIconStringParser implements StringParser{

    /* (non-Javadoc)
     * @see org.jpn.xucker.wiki.StringParser#canParse(java.lang.String)
     */
    public boolean canParse(String line) {
        return line.startsWith("<i ") || line.startsWith("<icon-");
    }
    ...
    public String parse(Element element, String line) {
        ...
        if(line.startsWith("<icon-")){
        	clazz=TagUtil.getName(all);
        }else{
        	 //System.out.println("all:"+all+","+line);
            clazz=(String)TagUtil.getAttribute(all).get("class");
             if(clazz==null){
                 throw new RuntimeException("i icon need class:"+line);
             }
             
        	//need?
        	 int start=all.indexOf(">");
             int end=all.indexOf("</i>",start);
             String content=null;
             if(end!=-1){
        ...
        IIcon alink=new IIcon();
        alink.addAttribute(clazz);
    ...
    public static IIconStringParser instance=new IIconStringParser();
    public static IIconStringParser getInstance(){
        return instance;
    }
    public IIconStringParser(){}
